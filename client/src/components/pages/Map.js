...
function Map() {
  ...
  return (
    <div id="map-container" className=" flex flex-col items-center justify-center ">
        ...
        <div className={`rounded-xl flex justify-center items-center py-6 my-4 h-11   text-center ${isAnimating ? 'animate-mapfade-out-words' : ''}`}
            style={{'--word-index': '0', '--out-index': '0'}}>
      ...
      {selectedRegion && (
        <button id="toggle-button" className={` bg-slate-500 hover:bg-slate-700 text-white font-bold py-3 px-16 rounded-full focus:outline-none focus:shadow-outline animate-mapfade-in-button ${isAnimating ? 'cursor-default disabled:pointer-events-none animate-mapfade-out-words ' : ''}`}
        ...
        disabled={isAnimating}  onClick={handleNext}>
         <span className={`button-text ${isAnimating ? 'animate-mapfade-out-words' : ''}`}
            style={{'--word-index': '0', '--out-index': '0'}}>View Babysitters Here</span>
        </button>
      )}
