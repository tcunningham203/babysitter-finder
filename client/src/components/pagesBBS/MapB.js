...
const MapB = () => {
  ...
  return (
    ...
    {selectedRegion && (
      <button id="toggle-button" className={` bg-slate-500 hover:bg-slate-700 text-white font-bold py-3 px-16 rounded-full focus:outline-none focus:shadow-outline animate-mapfade-in-button ${isAnimating ? 'cursor-default disabled:pointer-events-none animate-squish-button ' : ''}`}
      ...
      disabled={isAnimating}  onClick={handleNext}>
       <span className={`button-text ${isAnimating ? 'animate-mapfade-out-words' : ''}`}
          style={{'--word-index': '0', '--out-index': '0'}}>View Babysitters </span>
      </button>
    )}
