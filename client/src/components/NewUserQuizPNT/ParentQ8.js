  ...
  return (
    ...
    <div className="bg-slate-300 font-cool min-h-screen flex-col flex items-center justify-center">
     ...
     <h2 className={`flex items-center justify-center  animate-fade-in-word ${isAnimating ? 'animate-fade-out-words' : ''}`} style={{'--word-index': '1', '--out-index': '2'}}> <BearIcon/></h2>
          ...
          <button type="button" className={`items-center flex-col rounded-full bg-rose-700 pb-3 pt-3 w-14 leading-normal text-white transition duration-150 ease-in-out hover:bg-rose-800  focus:outline-none focus:ring-0 active:bg-rose-950 md:mt-16 mt-7 animate-fade-in-word ${isAnimating ? 'animate-fade-out-words' : ''}`} style={{'--word-index': '6', '--out-index': '4'}}>
        <VscMail size={33}/>
        </button>
          <h2 className={` md:mt-5 mt-1 animate-fade-in-word ${isAnimating ? 'animate-fade-out-words' : ''}`} style={{'--word-index': '6', '--out-index': '4'}}>Touch this to request contact. </h2>
          <h2 className={`md:mb-24 mt-1 animate-fade-in-word ${isAnimating ? 'animate-fade-out-words' : ''}`} style={{'--word-index': '6', '--out-index': '5'}}>If they're free, they will email you!</h2>
          <button
            type="submit"
          ...
          >
           <span className={`button-text ${isAnimating ? 'animate-fade-out-button' : ''}`}
            style={{'--word-index': '10', '--out-index': '0'}}>Got it!</span>
          </button>
        </form>
      </div>
    </div>
